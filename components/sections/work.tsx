    ...
    <>
      <div className="project-preview" style={style}>
        <div className="content">
          ...
          {workData.map(({ href, title, description, work }, i) => (
            <div
              key={i}
              className={cn("content__item", { active: i === activeIndex % workData.length })}
            >
              <h3>{title}</h3>
              <p>{description}</p>
              <h4>What we did:</h4>
        ...
        <div className="image-slider">
          ...
          <div className="image-slider__slidebar">
            <div
              className={cn("inner", { hideTransition })}
              style={{
                transform: `translateX(calc((0px - var(--phone-inner-width) - 36px) * ${activeIndex} - ${slideByTouch}px))`,
              }}
            >
              {[...workData, ...workData, ...workData].map(({ img }, i) => (
                <Fragment key={i}>
                  <picture
                    draggable
                    className={cn({ active: i === activeIndex, hideTransition })}
                    onTouchEnd={(e) => {
                      positionSlider(e);
                    }}
                  ...
                  >
                    <Image alt={img} height="1200px" src={img} width="248px" />
                  </picture>
                </Fragment>
              ))}
            </div>
      ...
      <style jsx>{`
        ...
        .phone {
          ...
          background: linear-gradient(90deg, var(--color-grey-bg-3), var(--color-grey-bg-3))
              content-box,
            linear-gradient(90deg, var(--color-grey-bg-3), var(--color-grey-bg-3)) border-box;
          box-shadow: 0 50px 100px -20px rgba(var(--color-text-faded-rgb), 0.25),
            0 30px 60px -30px rgba(var(--color-text-rgb), 0.3),
            inset 0 -2px 6px 0 rgba(var(--color-text-faded-rgb), 0.35);
        }
      `}</style>
    </>
  );
